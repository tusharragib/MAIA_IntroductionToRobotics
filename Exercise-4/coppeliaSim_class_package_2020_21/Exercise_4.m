...
q(:,1) = [   
   ...
   -0.0008
    1.2978
    1.5545    ]'; % approximated home configuration
...
quat_final = Rot2Quat([1 0 0; 0 0 -1; 0 1 0]);

for i=1:N
    
    [s,~,~] = trapezoidal(0, norm(p_f-p_i), 0.17, tf,t(i));
    xd(1:3,i) = center + Rot*rad*[cos(s/rad); sin(s/rad);0];
    %%%%%
    [theta,~,~] = trapezoidal(0, d_axes(4), 0.8, tf,t(i));
    an=[d_axes(1:3)'; theta]';
    Rd = axang2rotm(an);
    Rd = Ri*Rd;
